...
if __name__ == "__main__":
    ...
    # ------------------------------------------------------#
    input_shape = [600, 600]  # 裁剪图片统一为（600， 600）
    # ---------------------------------------------#
    #   可选两种主干：vgg resnet50
    # ---------------------------------------------#
    backbone = "resnet50"
    ...
    # ------------------------------------------------------------------#
    Freeze_Train = True  # 设置先进行冻结，再解冻训练
    ...
    if not pretrained:
        weights_init(model)
    if model_path != '':  # 如果设置了model_path，上面的pretrained就真的没有意义，因为会再次设置一遍初始值
        # ------------------------------------------------------#
        #   权值文件请看README，百度网盘下载
        # ------------------------------------------------------#
    ...
    model_train = model.train()  # 开启训练模式
    if Cuda:
