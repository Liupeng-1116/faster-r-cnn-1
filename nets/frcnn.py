...
class FasterRCNN(nn.Module):
    def __init__(self,
        ...
        if backbone == 'vgg':
            ...
            self.extractor, classifier = resnet50(pretrained)  # resnet50拆成的两部分
    ...
    def forward(self, x, scale=1., mode="forward"):
        if mode == "forward":
            ...
            #   计算输入图片的大小
            img_size = x.shape[2:]   # 取（N,C,H,W)中的（H,W)
#   -----------------第一阶段工作：主干网络，RPN网络--------------------   #
            #   利用主干网络提取特征
...
#   -----------------第二阶段工作：ROI层、多元类别分类、预测边界框偏移量--------------------   #
    ...
    def freeze_bn(self):   # freeze 冻结
        for m in self.modules():
            if isinstance(m, nn.BatchNorm2d):
                m.eval()   # 遇到batch-norm层，开启评估模式，冻结bn层
