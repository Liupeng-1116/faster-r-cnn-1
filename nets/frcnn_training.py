...
def bbox_iou(bbox_a, bbox_b):
    # 计算IOU
    if bbox_a.shape[1] != 4 or bbox_b.shape[1] != 4:  # 不满足左上右下四个坐标值格式要求
        print(bbox_a, bbox_b)
        raise IndexError

    tl = np.maximum(bbox_a[:, None, :2], bbox_b[:, :2])   # 左上角找最大的
    br = np.minimum(bbox_a[:, None, 2:], bbox_b[:, 2:])  # 右下角找最小的
    area_i = np.prod(br - tl, axis=2) * (tl < br).all(axis=2)  # 交集面积
    area_a = np.prod(bbox_a[:, 2:] - bbox_a[:, :2], axis=1)
    area_b = np.prod(bbox_b[:, 2:] - bbox_b[:, :2], axis=1)
    return area_i / (area_a[:, None] + area_b - area_i)  # 交并比


def bbox2loc(src_bbox, dst_bbox):   # 应该说是将真实框分配给锚框时的偏差转换计算
    # 或者说叫锚框偏移量转换
    width = src_bbox[:, 2] - src_bbox[:, 0]
    height = src_bbox[:, 3] - src_bbox[:, 1]
    ctr_x = src_bbox[:, 0] + 0.5 * width
    ctr_y = src_bbox[:, 1] + 0.5 * height  # 中心点坐标
    ...
    eps = np.finfo(height.dtype).eps   # 表示取得复合height.dtype数据类型的非负的最小值（极小）
    # 用来防止数值为负
    width = np.maximum(width, eps)  # 防止width为负
    height = np.maximum(height, eps)

    dx = (base_ctr_x - ctr_x) / width   # 计算锚框到真实框的偏移量
...
class AnchorTargetCreator(object):
    # 将真实边界框分配给锚框
    def __init__(self, n_sample=256, pos_iou_thresh=0.7,
                 neg_iou_thresh=0.3, pos_ratio=0.5):
        self.n_sample = n_sample   # 总样本数量
    ...
    def _calc_ious(self, anchor, bbox):
        ...
        # ---------------------------------------------------------#
        #   寻找与每一个真实框iou最大的锚框，也就是按照axis=1寻找
        #   或者说，先找每一列中iou最大的
    ...
    def _create_label(self, anchor, bbox):
        ...
        label.fill(-1)   # 用-1填充数组

        # ------------------------------------------------------------------------ #
        #   argmax_ious为每个锚框对应的最大的真实框的序号         [num_anchors, ]
        #   max_ious为每个锚框对应的最大的真实框的iou             [num_anchors, ]
        #   gt_argmax_ious为每一个真实框对应的最大的锚框的序号    [num_gt, ]
        # ------------------------------------------------------------------------ #
        ...
        #   如果小于门限值则设置为负样本（背景）
        #   如果大于门限值则设置为正样本
        #   每个真实框至少对应一个先验框，可以有多个先验框对应一个真实框
        # ----------------------------------------------------- #
        label[max_ious < self.neg_iou_thresh] = 0  # 小于负样本阈值，设置为背景
        label[max_ious >= self.pos_iou_thresh] = 1  # 大于正样本阈值，设置为正样本（物体）
        if len(gt_argmax_ious) > 0:
            label[gt_argmax_ious] = 1  # 只要是真实框寻找到的最大的锚框，就一定设置为正样本

        # ----------------------------------------------------- #
        #   判断正样本数量是否大于128，如果大于则限制在128（限制一个图片内真实框数量）
        # ----------------------------------------------------- #
        n_pos = int(self.pos_ratio * self.n_sample)  # 正样本数量
        pos_index = np.where(label == 1)[0]
        if len(pos_index) > n_pos:   # 超出限制
...
class ProposalTargetCreator(object):  # 对应上面，就是建议框分配真实框
    def __init__(self, n_sample=128, pos_ratio=0.5, pos_iou_thresh=0.5, neg_iou_thresh_high=0.5, neg_iou_thresh_low=0):
        self.n_sample = n_sample
        self.pos_ratio = pos_ratio
        self.pos_roi_per_image = np.round(self.n_sample * self.pos_ratio)  # 四舍五入
        self.pos_iou_thresh = pos_iou_thresh
        self.neg_iou_thresh_high = neg_iou_thresh_high  # 最高阈值
        self.neg_iou_thresh_low = neg_iou_thresh_low  # 最低阈值
    ...
    def __call__(self, roi, bbox, label, loc_normalize_std=(0.1, 0.1, 0.2, 0.2)):
        ...
        #   计算建议框和真实框的重合程度（即IOU）
        # ----------------------------------------------------- #
        iou = bbox_iou(roi, bbox)  # 建议框与真实框的IOU值
...
class FasterRCNNTrainer(nn.Module):
    ...
    def forward(self, imgs, bboxes, labels, scale):
        ...
        # -------------------------------------------------- #
        rpn_locs, rpn_scores, rois, roi_indices, anchor = self.model_train(x=[base_feature, img_size],
                                                                           scale=scale, mode='rpn')
        
        rpn_loc_loss_all, rpn_cls_loss_all, roi_loc_loss_all, roi_cls_loss_all = 0, 0, 0, 0
        # rpn边界框回归损失、 rpn二分类损失、 最终边界框回归损失、 候选区域分类损失
...
def get_lr_scheduler(lr_decay_type, lr, min_lr, total_iters,
                     warmup_iters_ratio=0.05, warmup_lr_ratio=0.1, no_aug_iter_ratio=0.05, step_num=10):
...
def set_optimizer_lr(optimizer, lr_scheduler_func, epoch):
    lr = lr_scheduler_func(epoch)  # lr的更新以及设置替换
